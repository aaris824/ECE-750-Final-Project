...
// Global variables for head motor devices
WbDeviceTag headYaw, headPitch;

// Global declarations for hip motors
WbDeviceTag lHipYawPitch, lHipRoll, lHipPitch, rHipYawPitch, rHipRoll, rHipPitch;
...
static void find_and_enable_devices() {
  ...
  // Initialize hip motors for torso movement
  ...
  // For NAO, both legs share the same HipYawPitch motor, so this might be unnecessary:
  ...
  // Ensuring all devices were successfully initialized
  if (!lHipYawPitch || !lHipRoll || !lHipPitch || !rHipRoll || !rHipPitch) {
    printf("Error: Can't find one or more hip motors.\n");
  }
...
static void passive_wait(double duration) {
  double startTime = wb_robot_get_time();
  while (wb_robot_get_time() < startTime + duration) {
...
int main() {
    ...
    // Get the device tags for motors
    ...
    WbDeviceTag rShoulderPitch = wb_robot_get_device("RShoulderPitch");
    WbDeviceTag lShoulderRoll = wb_robot_get_device("LShoulderRoll");
    WbDeviceTag rShoulderRoll = wb_robot_get_device("RShoulderRoll");
    WbDeviceTag lElbowYaw = wb_robot_get_device("LElbowYaw");
    WbDeviceTag rElbowYaw = wb_robot_get_device("RElbowYaw");

    // Initialize motor positions
    double yawAngle = 0.5; // Example angle for HeadYaw
    double pitchAngle = 0.2; // Example angle for HeadPitch

    // Main loop
    while (wb_robot_step(time_step) != -1) {
        // Simulate head movement
        wb_motor_set_position(headYaw, yawAngle);
        wb_motor_set_position(headPitch, pitchAngle);

        // Adjust arm positions for engagement
        wb_motor_set_position(lShoulderPitch, 0.24);
        wb_motor_set_position(rShoulderPitch, 0.24);
        wb_motor_set_position(lShoulderRoll, 0.0);
        wb_motor_set_position(rShoulderRoll, 0.0);
        wb_motor_set_position(lElbowYaw, -1.5);
        wb_motor_set_position(rElbowYaw, 1.5);

        // Toggle the head movement direction for gesture
        yawAngle = -yawAngle; // Toggle between looking left and right
        pitchAngle = -pitchAngle; // Toggle between looking up and down
