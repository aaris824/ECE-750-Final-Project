...
/*************************** START OF MY CODE MODIFICATIONS ***********************/ 

// Note some of the lines of code were from the create demo. I basically
//  just made this robot go forward for my study design, in video A3
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    if (is_there_a_collision_at_left() || is_there_a_cliff_at_left()) {
      printf("Obstacle detected on the left. Backing up and turning.\n");
      go_backward();
      passive_wait(0.5); // Back up for half a second
      turn(M_PI * 0.25); // Turn slightly to the right
    } else if (is_there_a_collision_at_right() || is_there_a_cliff_at_right() || is_there_a_cliff_at_front()) {
      printf("Obstacle detected on the right. Backing up and turning.\n");
      go_backward();
      passive_wait(0.5); // Back up for half a second
      turn(-M_PI * 0.25); // Turn slightly to the left
    } else {
      go_forward(); // Mainly move forward
    }
